...
setwd('measurements/20210503_T15')
getwd()
...
setwd('..')
getwd()
write.csv(x=dat_nested$ID_sort,file="constraints_T15.csv")

setwd('..')
setwd('plots/20210503_T15')
...
setwd('measurements')
getwd()
constraints <- read_excel("constraints_T15.xlsx", sheet = "constraints_T15", col_names = TRUE)
...
MR_plot        <- read_excel("processed_summary.xlsx", sheet = "summary", col_names = TRUE)
MR_plot$ploidy <- factor(MR_plot$ploidy, levels=c("D","T"),ordered=TRUE)
MR_plot$temp   <- factor(MR_plot$temp, levels=c("10","30"),ordered=TRUE)

summary(my_anova <- aov(umol_L_hr ~ temp*ploidy, data = MR_plot))
...
## Paired plot
MR_plot        <- read_excel("MR_paired.xlsx", sheet = "35-40", col_names = TRUE)
MR_plot$p      <- factor(MR_plot$p, levels=c("D","T"),ordered=TRUE)
# MR_plot$temp   <- factor(MR_plot$temp, levels=c("10","20","30","35","40"),ordered=TRUE)


p1 <- ggplot(MR_plot, aes(x=timepoint, y=umol_L_hr,group=as.factor(ID),color=ploidy)) +
      geom_point() +
      geom_line() +
