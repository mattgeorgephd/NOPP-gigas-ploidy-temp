...
quickview
#based on controls, I would subtract Repeat 10 values from Repeat 1 values without subtracting 0s from dataset
####Extract Data
...
#I chose to redo Samples R60, N49, N57, X47, X48, N41, R62, M46, N47 due to negative values or flat curves

###Get CS results###
time_i<-subset(CS_2, CS_2$Repeat==1)
time_f<-subset(CS_2, CS_2$Repeat==10) #pick maximum data point
initial<-time_i %>% 
  group_by(Label) %>%
...
final<-time_f %>% 
  ...
  summarise(meanA = mean(A))
#####Calculate Delta OD
final<-final[-c(2),] #this will change each time
initial<-initial[-c(2),] #this will change each time
...
bsa<-read.csv(file="https://raw.githubusercontent.com/mattgeorgephd/NOPP-gigas-ploidy-temp/main/202107_EXP2/citrate_synthase/Raw%20Data/BSA%203.22.22%2024-48.csv")
names(bsa)[6]<-'A'
w<-c("N42", "N42", "N42", "T55", "T55", "T55", "M48", "M48", "M48", "X47", "X47", "X47", "X47", "X47", "X47", "N43", "N43", "N43", "M90",	"M90",	"M90", "R58", "R58", "R58", "R60", "R60", "R60", "R78", "R78", "R78", "N49", "N49", "N49", "M42", "M42", "M42", "N59","N59", "N59", "N57", "N57", "N57", "M49", "M49", "M49", "D54", "D54", "D54", "T57", "T57", "T57", "X45", "X45", "X45", "N41", "N41", "N41", "X48", "X48", "X48", "M43", "M43", "M43", "X41", "X41", "X41",  "M46", "M46", "M46", "N47", "N47", "N47", "R62", "R62", "R62", "empty", "empty", "empty")
