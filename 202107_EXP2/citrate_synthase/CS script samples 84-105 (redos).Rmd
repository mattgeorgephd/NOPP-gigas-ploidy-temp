...
#wells<-c(0, 8, 16, 24, 32, 40, "pos","pos", "pos", "background", "background", "background", "X47", "X47", "X47", "T61", "T61", "T61", "X42", "X42", "X42", "N57", "N57", "N57", "X48", "X48", "X48", "R42", "R42", "R42", "X43", "X43", "X43", "X44", "X44", "X44", "D55", "D55", "D55", "M47", "M47", "M47", "N44", "N44", "N44", "D59", "D59", "D59", "R51", "R51", "R51", "T56", "T56", "T56", "R60", "R60", "R60", "M41", "M41", "M41", "R46", "R46", "R46", "M89",	"M89",	"M89", "T58", "T58", "T58", "R48", "R48", "R48", "T62", "T62", "T62") 

CS_6$Label<-rep(wells, 15) #note rep=15 
CS_6['A2'] <- NA

###View controls
controls<-c(0,'background', 'pos', 'R60')
...
#based on controls, I would subtract Repeat 15 values from Repeat 1 values with doing a background control correction

background_1<-0.11 #subtract from time_i
background_15<-0.085 #subtract from time_f

# Make new CS standard curve for new data (7.12.2022)
cs_standards<-CS_6[c(1:6),] #set standards and removed outliers
standards<-as.numeric(c(0, 8, 16, 24, 32, 40)) #relabel
cs_standards$GSH<-standards
cs_standards$A2<-cs_standards$A-background_1

#plot CS standards
standard_plot<-ggplot(data=cs_standards,(aes(x=A2, y=GSH)))+geom_point()+geom_smooth(method="lm", se=FALSE, formula = y ~ poly(x, 1) )+ylab("Concentration of standard ug/mL")+xlab("A @ 570nm")+theme_bw()+stat_regline_equation(label.x=0.25, label.y=20)
standard_plot
...
time_i<-subset(CS_6, CS_6$Repeat==1) #pick minimum data point
time_f<-subset(CS_6, CS_6$Repeat==15) #pick maximum data point

time_i$A2<-time_i$A-background_1
time_f$A2<-time_f$A-background_15

initial<-time_i %>% 
  group_by(Label) %>%
  summarise(meanA = mean(A2))
final<-time_f %>% 
  group_by(Label) %>%
  summarise(meanA = mean(A2))
...
BSA_results<-mutate(samples_BSA, protein_content, P)

######Calculate Final CS Values
#y= 37x -1.8 (from CS activity plot)
t<-62 #min
m<-37
b<--1.8
GSH<-(m*deltaOD)+b 
...
results<-mutate(initial, OD=deltaOD, GSH=GSH, protein_content=protein_content, "Protein (mg/ul)"=P, "CS activity (nmol/(mg*min))"=CS_activity) #OD is the delta OD for the samples
...
#R60 is negative again, set to zero instead
results_positive <- results_joined                   # Duplicate data frame
results_positive[results_positive < 0] <- 0     # Set negative values to 0
write.csv(results_joined, file="/Users/oliviacattau/Documents/GitHub/NOPP-gigas-ploidy-temp/202107_EXP2/citrate_synthase/results/result_105.csv")

final_plot<-ggplot(data=results_positive,(aes(x=results_positive$`Protein (mg/ul)`, y=results_positive$`CS activity (nmol/(mg*min))`, color=trt)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE, formula = y ~ x ) + ylab("CS Activity (nmol/(mg*min))")
final_plot
```
