...
boxplot<-ggplot(data=results_unique, xaxt="n")+geom_boxplot(aes(x=trt, y=CS.activity..nmol..mg.min.., color=ploidy))+theme_bw()+ylab("CS Activity (nmol/(mg*min)")+xlab("Treatment Group")
...
#ANOVA test
#for three or more samples = k
#are any of the means of the samples significantly different?
#by hand
library(plyr)
daply(results_unique, .(trt, results_unique$CS.activity..nmol..mg.min..), function(trt), trt$trt)
mean_1<-mean(results_unique$
mean_2<-mean(ReducingPain$Music)
mean_3<-mean(ReducingPain$Earphones)
mean_total<-mean(ReducingPainLong$value)
k<-3
N<-length(ReducingPainLong$value)
n_1<-length(ReducingPain$Audiobook)
n_2<-length(ReducingPain$Music)
n_3<-length(ReducingPain$Earphones)
calc.total.ANOVA<-function(ReducingPainLong) {
  sum.squares<-(sum((ReducingPainLong$value-mean(ReducingPainLong$value))^2))
  df<-length(ReducingPainLong$value)-1
  return(list(Total_SS=sum.squares, df=df))
}
calc.total.ANOVA(ReducingPainLong) #Total SS
means<-c(mean_1, mean_2, mean_3)
n<-c(n_1, n_2, n_3)
SS_group<-sum(n*(means-mean_total)^2) #Group SS
SS_total<-
DF_total<-39
SS_group<-
DF_group<-3
...
1-pf(F_stat, df1=DF_group, df2=DF_error)
#one tailed, right hand only
qf(p=0.05, df1=k-1, df2 = N-k, lower.tail = F)
pf(q=10.13, df1=k-1, df2=N-k, lower.tail = F)
#checking our assumptions
var(data)
vars<-apply(X=X, MARGIN = 2, FUN = var)
max(vars)/min(vars)
```
