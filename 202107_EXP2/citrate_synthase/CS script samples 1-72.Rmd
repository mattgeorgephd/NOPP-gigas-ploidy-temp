...
-Shell Height
-Shell Volume 
-Calculated Dry Weights 
-Ploidy (triploid/diploid)
-Heat (control/heat/desiccation) is trt
-Heat + Ploidy is ploidy.trt
-Sample Date (7.12.21 or 7.13.21) or dissection date
-Date of Sample (3.22.22 or 7.11.22) or CS assay date
...
#direct GLM
morphANOVA<-lm(filtered_morph$CS.activity..nmol..mg.min..~filtered_morph$ploidy.trt+ filtered_morph$shell_volume+filtered_morph$shell_length+filtered_morph$shell_width+filtered_morph$shell_height+filtered_morph$sample_date+filtered_morph$calc_dry_weight+filtered_morph$ploidy+filtered_morph$trt+filtered_morph$date.of.sample+filtered_morph$tank, digits=4)

anova(morphANOVA)#trt, ploidy+trt, and sample_date are all significant while all shell morphometrics are not

table2<-anova(morphANOVA)

#Trt and Date
trt_date<-ggplot(filtered_morph, aes(fill=trt, x=sample_date))+geom_bar()+theme_bw()
trt_date #non-random distribution, pull it out of analysis 

#interactions
morphANOVA2<-lm(filtered_morph$CS.activity..nmol..mg.min..~filtered_morph$ploidy.trt* filtered_morph$shell_volume*filtered_morph$shell_length+filtered_morph$shell_width*filtered_morph$shell_height*filtered_morph$calc_dry_weight*filtered_morph$ploidy*filtered_morph$trt*filtered_morph$date.of.sample*filtered_morph$tank)

anova(morphANOVA2) #calculated dry weight * date of sample are significant 

table3<-anova(morphANOVA2)

summary(morphANOVA2)$r.squared #0.98!
...
#Linear Mixed Models
## Linear Model
fit_1<-lm(filtered_morph$CS.activity..nmol..mg.min..~filtered_morph$day)
plot(fit_1, 5)
summary(fit_1)
summary(fit_1)$r.squared #0.33
coef(fit_1)
plot(residuals(fit_1))
qqnorm(residuals(fit_1))#errors do not appear to be skewed but they are heavy-tailed 
qqline(residuals(fit_1))
acf(residuals(fit_1))

fit_2<-lm(filtered_morph$CS.activity..nmol..mg.min..~filtered_morph$sample_date+filtered_morph$calc_dry_weight)
plot(fit_2, 5) #remove 57 (R53)
summary(fit_2)
summary(fit_2)$r.squared# 0.34

new_morph_data<-filtered_morph[-c(57),]
fit_3<-lm(new_morph_data$CS.activity..nmol..mg.min..~new_morph_data$sample_date+new_morph_data$calc_dry_weight+new_morph_data$ploidy.trt+new_morph_data$trt)#best simplified model
plot(fit_3, 5) 
plot(fit_3, 4) 
summary(fit_3) 
summary(fit_3)$r.squared #0.44
```

## Data Visualization 
```{r}
#### All plots
boxplot<-ggplot(data=results_unique, aes(x=factor(trt), y=CS.activity..nmol..mg.min..))+
  geom_boxplot(aes(x=factor(trt), y=CS.activity..nmol..mg.min.., color=ploidy))+
  theme_bw()+
...
cdw<-ggplot(filtered_morph, aes(y=calc_dry_weight, x=trt))+geom_boxplot()+theme_bw()+ylab("Calculated Dry Weight")
cdw
```

# Results
- sample_date is significant (one extra day before sampling)
- trt is almost significant at 0.06
- ploidy trt is almost significant at 0.08
- sample_date:calc_dry_weight interaction is significant at 0.02
