...
#based on controls, I would subtract Repeat 10 values from Repeat 2 values with subtracting background from samples 

####Extract Data
samples<-c("R46", "M89", "M41", "D55", "D56", "X42", "T62", "T59", "D58",   "R50",  "R42",  "R48",  "T56",  "X44",  "R47", "N45", "N44", "X43", "M47",  "T58", "N46", "T61", "D59")
samples_x<-c("D56", "R42", "R47", "T58", "T59", "T61") #out of range, remove
mydata<-filter(CS, CS$Label %in% samples)
mydata_x<-filter(CS, CS$Label %in% samples_x)

#view data
quickview2<-ggplot(data=mydata_x, aes(x=Repeat, y=A, color=(Label)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE, formula=y~x)+scale_x_continuous(breaks=seq(1, 10, by=1))
quickview2 #messy data, might need to redo 
