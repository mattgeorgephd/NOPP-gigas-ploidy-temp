...
mydata<-filter(CS, CS$Label==samples)
#view data
quickview2<-ggplot(data=positive, aes(x=mydata, y=A, color=Label))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE, formula=y~x)
quickview2
####get CS results
...
samples<-bsa_standards_2[c(9:77),]
A<-bsa %>% 
  group_by(Label) %>%
  summarise(meanA = mean(A))
A_samples<-A[-c(1:8, 24),]
