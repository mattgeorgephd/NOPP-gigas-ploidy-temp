...
filter_my_data<-filter_my_data[-c(3,9,13,15,19)] #remove columns without data
#keep: ploidy, trt, shell_length, shell_width, shell_height, mortality, shell volume, cal_dry_weight, ploidy.trt, tank

Full_data<-left_join(filter_my_data, Full_dataset, by="ID")

Full_data #complete dataset

#add mortality data
#mortality
final_mort<-c(94.318, 90.909, 85.227, 77.272, 90.909, 63.636) #calculated from Master File
final_list<-c("D-control", "D-heat", "D-desiccation", "T-control", "T-heat", "T-desiccation")
mortality<-data.frame(final_mort, final_list)
```


#Data Visualization
```{r}
final_plot<-ggplot(data=Full_data,(aes(x=protein, y=CS_activity, color=ploidy)))+geom_point()+theme_bw()+geom_smooth(method="lm", se=FALSE, formula = y ~ x ) + ylab("CS Activity (nmol/(mg*min))") +xlab("Protein (ug/mL)")
