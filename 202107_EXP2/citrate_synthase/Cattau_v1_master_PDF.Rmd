...
```{r}
CS_background<-read.csv(file="/Users/oliviacattau/Documents/GitHub/NOPP-gigas-ploidy-temp/202107_EXP2/citrate_synthase/Raw Data/CS_12_10_22_background.csv") 
...
#Statisical Analysis
for verifying results in boxplot HSD and t-tests
```{r}
...
```{r}
small_model<-aov(CS_activity~ploidy*trt*P, data=Full_data3) #point 59 is an outlier
large_model<-aov(CS_activity~ploidy+trt+tank+sample_date+shell_volume+day, data=Full_data3)#trt=treatments are the only variable that is significant
combinedANOVA<-lm(CS_activity~trt, data=Full_data3)
car::Anova(combinedANOVA, type=3)
...
mortality2$sig_groups<-c("b", "a", "b", "b", "a", "b")
...
```
Based on the HSD (Honestly Significantly Different Test) Triploid desiccation and Diploid desiccation groups are significantly different (and elevated in CS enzyme activity) relative to their control and heat only groups. 
