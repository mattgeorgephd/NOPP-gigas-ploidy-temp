...
boxplot3<-ggplot(data=Full_data3, aes(x=factor(trt_short), y=CS_activity))+
  geom_boxplot(aes(x=factor(trt_short, level=c("control", "heat", "heat + desiccation")), y=CS_activity))+
  theme_minimal()+geom_point(data=Full_data3, alpha=0.5)+
  ylab(expression('CS activity nmol' (min^-1) (mg^-1)))+
  xlab("Treatment Group") + scale_y_continuous(limits=c(0,8))+facet_wrap(~ploidy2)+scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
boxplot3
