...
#Statisical Analysis
for verifying results in boxplot with F-tests and for regression analysis
```{r}
...
#model1<-lmer(formula = CS_activity ~ 1 + trt + ploidy + calc_dry_weight +(1|tank), data=Full_data3)
...
#Table 1 below
combined.lsm<-emmeans::emmeans(combinedANOVA, pairwise~trt)
combined.contrasts<-emmeans::contrast(combined.lsm, "eff")
combined.contrasts

#multi-variant compairison
...
```
Based on the HSD (Honestly Significantly Different Test) Triploid Desiccation and Diploid desiccation groups are significantly different (and elevated in CS anzyme activity) relative to their control and heat only groups. 
