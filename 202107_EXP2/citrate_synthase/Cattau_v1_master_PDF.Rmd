...
```{r}
supp1<-read.csv(file="https://raw.githubusercontent.com/ocattau/CS-manuscript/main/raw-data/supp1.csv")
...
large_model<-aov(nmol~BSA+ploidy+trt+tank+sample_date+shell_volume+day+heat, data=supp1)
...
resp_plot<-ggplot(data=respdata, aes(x=avgRES, y=avgCS))+geom_point(aes(color=group_list), size=5)+theme_minimal()+geom_smooth()
```
