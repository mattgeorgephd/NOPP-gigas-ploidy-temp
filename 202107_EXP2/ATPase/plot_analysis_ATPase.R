...
trt_list <- read_excel("ATPase.xlsx", sheet = "trt_list", col_names = TRUE)
...
library(multcomp)
tx <- with(HEAT_plot_ideal, interaction(timepoint,ploidy))
amod <- aov(ATPase ~ tx, data=HEAT_plot_ideal)
HSD.test(amod, "tx", group=TRUE, console=TRUE)

postHocs <- glht(ancova_model, linfct = mcp(combo = "Tukey"))
summary(postHocs)
postHocs <- glht(ancova_model, linfct = mcp(day = "Tukey"))
summary(postHocs)
