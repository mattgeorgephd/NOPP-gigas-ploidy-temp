...
load.lib<-c("DESeq2","edgeR","goseq","dplyr","GenomicFeatures","data.table","calibrate","affycoretools","data.table","vsn","tidybulk","ggplot2","cowplot","pheatmap","gplots","RColorBrewer","EnhancedVolcano","pcaExplorer","readxl","apeglm","ashr","tibble","plotly")
...
coldata$group <- factor(trt_list$group) # for individual pair-wise comparisions

# Remove bad samples - N56, X44 (see MultiQC report)
coldata <- coldata[!(row.names(coldata) %in% c("N56","X44")),]
cts <- as.matrix(subset(cts, select=-c(N56,X44)))
...
# Remove genes with less than 100 counts across samples 
keep <- rowSums(cts) >= 100
cts_filtered <- cts[keep,]
print(paste("# genes remaining before pre-filtering:", nrow(cts)))
print(paste("# genes remaining after pre-filtering:", nrow(cts_filtered)))
print(paste("# of genes dropped:", nrow(cts) - nrow(cts_filtered), sep=" "))
```
...
# summary(res_all)
summary(res_ploidy)                       # up (13%), down (12%)
# summary(res_cond_heat)                    # up (6.2%), down (4.2%)
# summary(res_cond_des)                     # up (3.4%), down (2.9%)

# sum(res_all$padj < 0.1, na.rm=TRUE)       # 7551
sum(res_ploidy$padj < 0.1, na.rm=TRUE)    # 7551
# sum(res_cond_heat$padj < 0.1, na.rm=TRUE) # 3164
# sum(res_cond_des$padj < 0.1, na.rm=TRUE)  # 1925
