...
# write.xlsx(x=dat_nested$ID_sort, file="constraints_T10.xlsx", sheetName = "T10", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/T10'); getwd()
...
constraints <- read_excel("constraints_T10.xlsx", sheet = "T10", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_slope_T10.xlsx", sheetName = "T10", col.names = TRUE, row.names = TRUE, append = FALSE)
