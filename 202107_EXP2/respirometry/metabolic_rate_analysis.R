...
setwd('constraints')
getwd()
write.xlsx(x=dat_nested$ID_sort, file="constraints_T6.xlsx", sheetName = "T6", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/T6'); getwd()
...
constraints <- read_excel("constraints_T6.xlsx", sheet = "T6", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_slope_T6.xlsx", sheetName = "T6", col.names = TRUE, row.names = TRUE, append = FALSE)
