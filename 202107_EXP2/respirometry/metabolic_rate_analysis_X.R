...
write.xlsx(x=dat_nested$ID_sort, file="constraints_X.xlsx", sheetName = "X", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/X'); getwd()
...
constraints <- read_excel("constraints_X.xlsx", sheet = "X", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_X.xlsx", sheetName = "X", col.names = TRUE, row.names = TRUE, append = FALSE)
