...
getwd()
setwd("measurements/heat_only_T2")
...
write.xlsx(x=dat_nested$ID_sort, file="constraints_heat_only_T2.xlsx", sheetName = "heat_only_T2", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/heat_only_T2'); getwd()
...
constraints <- read_excel("constraints_heat_only_T2.xlsx", sheet = "heat_only_T2", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_slope_heat_only_T2.xlsx", sheetName = "heat_only_T2", col.names = TRUE, row.names = TRUE, append = FALSE)
