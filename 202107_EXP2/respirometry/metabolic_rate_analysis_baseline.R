...
setwd('constraints')
getwd()
write.xlsx(x=dat_nested$ID_sort, file="constraints_extra.xlsx", sheetName = "extra", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/extra'); getwd()
...
constraints <- read_excel("constraints_extra.xlsx", sheet = "extra", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_slope_extra.xlsx", sheetName = "extra", col.names = TRUE, row.names = TRUE, append = FALSE)
