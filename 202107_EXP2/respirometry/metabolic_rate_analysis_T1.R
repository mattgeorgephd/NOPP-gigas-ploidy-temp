...
setwd('constraints')
getwd()
# write.xlsx(x=dat_nested$ID_sort, file="constraints_T1.xlsx", sheetName = "T1", col.names = TRUE, row.names = TRUE, append = FALSE)


# Plot the lines one by one to find ones to trash. Add start and stop to for recalcs in constraints tab of MR_output.

getwd(); setwd('..'); setwd('QC_plots/T1'); getwd()
...
constraints <- read_excel("constraints_T1.xlsx", sheet = "T1", col_names = TRUE)
...
write.xlsx(x=oxy_hr,file="SMR_slope_T1.xlsx", sheetName = "T1", col.names = TRUE, row.names = TRUE, append = FALSE)
